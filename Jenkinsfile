pipeline {
    ...
    environment {
        IMAGE_NAME = "abakhar217/notification-service:notification-service-${BUILD_NUMBER}"
        ...
        stage('Checkout') {
            steps {
                ...
                checkout([$class: 'GitSCM',
                          ...
                          userRemoteConfigs: [[url: 'https://github.com/InvestBuddy/notification-service.git', credentialsId: 'git']]])
            }
        }
        ...
        stage('Check if JAR exists') {
            steps {
                script {
                    ...
                    if (!fileExists('target/notification-service-1.0-SNAPSHOT.jar')) {
                        error "notification-service-1.0-SNAPSHOT.jar not found! Build failed."
                    }
                }
            }
        }
